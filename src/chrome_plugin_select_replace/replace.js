...
var inclNodeIds = ['project_version', 'cycle_names', 'addTestsVersion', 'addTestsCycle'];
// Limit replacement to these parent classes
var inclParentClasses = '.jira-dialog,.aui-dialog';
		...
		mutations.forEach(function(mutation) {
			...
			mutation.addedNodes.forEach(function(node) {
				$(node).find('select').each(function() {
					...
					if ($(this).parents('.jira-dialog,.aui-dialog').length <= 0) {
						return;
					}
